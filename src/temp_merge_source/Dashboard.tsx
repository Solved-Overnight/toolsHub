...
import { 
  ...
  ArrowRight, Droplets, Leaf, Gauge, PieChart as PieChartIcon, AlertCircle
} from 'lucide-react';
...
const SummaryStat: React.FC<{ label: string; value: number; icon: any }> = ({ label, value, icon: Icon }) => (
  <div className="p-2.5 bg-app-bg rounded-md border border-app-border/50 text-center hover:bg-app-accent/5 transition-colors">
    <div className="flex items-center justify-center gap-1 mb-1 opacity-60">
      ...
      <span className="text-[8px] font-black text-app-text-muted uppercase tracking-tight">{label}</span>
    </div>
    <p className="text-xs font-black text-app-text">{(value/1000).toFixed(1)}k <span className="text-[8px] font-normal opacity-50">kg</span></p>
  </div>
);
