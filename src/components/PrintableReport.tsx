          ...
          <table className={`w-full border-collapse ${borderStyle} mb-4`}> {/* Apply border style */}
            ...
            <tbody>
              {chemicalItems.map((item, index) => {
                ...
                const itemNameDisplay = isDyeingStep ? '-------' : (item.itemName || 'N/A');
                const itemNameAlignmentClass = itemNameDisplay === '-------' ? 'text-center' : 'text-left';

                return (
                  <tr key={index} style={{ fontWeight: item.highlight ? 'bold' : 'normal' }}>
                    <td className={dataCellStyle}>{item.itemType || 'N/A'}</td>
                    <td className={`${borderStyle} ${itemNameAlignmentClass}`}>{itemNameDisplay}</td>
                    <td className={dataCellStyle}>{isDyeingStep ? '-------' : (item.lotNo || 'N/A')}</td>
                    <td className={`${borderStyle} text-center`}>{isDyeingStep ? '-------' : (item.dosing !== null ? item.dosing.toFixed(2) : '')}</td>
                    <td className={`${borderStyle} text-center`}>{isDyeingStep ? '-------' : (item.shade !== null ? item.shade.toFixed(2) : '')}</td>
                    <td className={dataCellStyle}>{kg}</td>
                    <td className={dataCellStyle}>{gm}</td>
                    <td className={dataCellStyle}>{mg}</td>
                    <td className={dataCellStyle}>{isDyeingStep ? '-------' : (item.unitPrice !== null ? item.unitPrice.toFixed(2) : '')}</td>
                    <td className={dataCellStyle}>{isDyeingStep ? '-------' : (item.costing !== null ? item.costing.toFixed(2) : 'N/A')}</td>
                    <td className={dataCellStyle}>{isDyeingStep ? '-------' : (item.remarks || '')}</td>
                  </tr>
                );
              })}
              {/* Empty rows */}
