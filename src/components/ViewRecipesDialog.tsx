...
import { Search, FolderOpen, Loader2, FileText, Beaker, Clock, Trash2, Download, X } from 'lucide-react';
import { db } from '../lib/firebaseConfig';
import { collection, query, orderBy, onSnapshot } from 'firebase/firestore';
import { getAuth, EmailAuthProvider, reauthenticateWithCredential } from 'firebase/auth'; // Import Firebase Auth
import type { Recipe, DyeingFormData, ChemicalItem } from '../types';
import { ProformaInvoiceData } from '../types/invoice';
import { useToast } from './ui/ToastProvider';
import { PasswordInputDialog } from './PasswordInputDialog'; // Import PasswordInputDialog
...
interface ViewRecipesDialogProps {
  ...
  collectionPath: string; // e.g., "recipes", "invoices"
...
export function ViewRecipesDialog({ isOpen, onClose, onRetrieve, onDelete, user, collectionPath, itemType }: ViewRecipesDialogProps) {
  ...
  const handleConfirmDelete = async (password: string) => {
    ...
    try {
      ...
      const currentUser = authInstance.currentUser; // Get the most current user

      if (!currentUser) {
        console.error("Authentication Error: No authenticated user found for reauthentication.");
        throw new Error("No authenticated user found for reauthentication.");
      }

      // Log current user's provider data for debugging
      console.log("Current user provider data:", currentUser.providerData);

      // Ensure currentUser.email is available for reauthentication
      ...
      console.log("Attempting reauthentication for user:", currentUser.email);

      const credential = EmailAuthProvider.credential(currentUser.email, password);
      await reauthenticateWithCredential(currentUser, credential); // Use currentUser here

      console.log("Reauthentication successful.");

      // If re-authentication is successful, proceed with deletion
      onDelete(recipeToDelete.id, recipeToDelete.name); // Call parent's onDelete
      showToast({
      ...
      console.error("Authentication or Deletion Error:", error);
      ...
      } else if (error.message === "No authenticated user found for reauthentication.") {
        errorMessage = "No active user session. Please log in again.";
      } else if (error.code === 'auth/invalid-credential') {
        errorMessage = "Invalid credential provided. This might happen if the user's authentication provider is not email/password.";
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      <PasswordInputDialog
        ...
        message={`Please enter your password to confirm deletion of "${recipeToDelete?.name || 'this item'}". This action cannot be undone.`}
