...
import { ProductionAnalyzerData } from './pages/ProductionAnalyzerData';
...
function App() {
  ...
  // Temporary state for production records until valid firebase connection or state management is implemented
  const [productionRecords, setProductionRecords] = useState<ProductionRecord[]>([]);

  const handleAddRecord = (record: ProductionRecord, replaceId?: string) => {
    setProductionRecords(prev => {
      if (replaceId) {
        return prev.map(r => r.id === replaceId ? record : r);
      }
      return [record, ...prev];
    });
  };
                ...
                <Routes location={location} key={location.pathname}>
                  {user ? (
                    <>
                      ...
                      <Route path="/production-dashboard" element={<motion.div
                        initial={{ opacity: 0, x: 20 }}
                        animate={{ opacity: 1, x: 0 }}
                        exit={{ opacity: 0, x: -20 }}
                        transition={{ duration: 0.3 }}
                      ><ProductionDashboard records={productionRecords} /></motion.div>} />
                      <Route path="/production-analyzer" element={<motion.div
                        initial={{ opacity: 0, x: 20 }}
                        animate={{ opacity: 1, x: 0 }}
                        exit={{ opacity: 0, x: -20 }}
                        transition={{ duration: 0.3 }}
                      ><ProductionAnalyzerData
